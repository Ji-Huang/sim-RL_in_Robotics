...
def compute_projection(points_3D, rotation, translation, internal_calibration):
    distCoeffs = np.zeros((8, 1), dtype='float32')
    projections_2d = cv2.projectPoints(points_3D, rotation, translation, internal_calibration, distCoeffs)[0]
    projections_2d = projections_2d.reshape(-1, 2)
...
def get_3D_corners(vertices):
    ...
    #corners = np.concatenate((corners, np.ones((9, 1))), axis=1)
    return corners
...
def corner_confidences(gt_corners, pr_corners, th=80, sharpness=2, im_width=640, im_height=480):
    ''' gt_corners: Ground-truth 2D projections of the 3D bounding box corners, shape: (16 x nA), type: torch.FloatTensor
        ...
        -----------
        return    : a torch.FloatTensor of shape (nA,) with nA confidence values
    '''
    nA = gt_corners.shape[1]
    ...
    eps = 1e-5
    distthresh = torch.FloatTensor([th]).repeat(nA, num_keypoints).to(device)
    dist = torch.sqrt(torch.sum(dist ** 2, dim=2)).squeeze() # nA x 9
    mask = (dist < distthresh).type(torch.FloatTensor).to(device)
    ...
    conf = conf / conf0.repeat(1, num_keypoints).to(device)
...
def corner_confidence(gt_corners, pr_corners, th=80, sharpness=2, im_width=640, im_height=480):
    ...
    '''
    device = pr_corners.device
    dist = torch.FloatTensor(gt_corners).to(device) - pr_corners
    ...
    conf  = conf / conf0.repeat(num_keypoints, 1).to(device)
    conf  = mask * conf 
...
def get_region_boxes(output, num_classes, num_keypoints, only_objectness=1, validation=True):
    ...
    # Activation
    output = (output.view(batch * anchor_dim, 2 * num_keypoints + 1 + num_classes, h * w)
              .transpose(0, 1).contiguous().view(2 * num_keypoints + 1 + num_classes, batch * anchor_dim * h * w))
    grid_x = (torch.linspace(0, w-1, w).repeat(h, 1)
              .repeat(batch * anchor_dim, 1, 1).view(batch * anchor_dim * h * w).to(device))
    grid_y = (torch.linspace(0, h-1, h).repeat(w, 1).t()
              .repeat(batch * anchor_dim, 1, 1).view(batch * anchor_dim * h * w).to(device))
    ...
    cls_confs = torch.nn.Softmax()(output[2 * num_keypoints + 1: 2 * num_keypoints + 1 + num_classes].transpose(0, 1))
    ...
    for b in range(batch):
        ...
        # For each output, select the cell with highest confidence
        for cy in range(h):
            for cx in range(w):
        ...
        # multiobject with different classes are not considered here
    return all_boxes
