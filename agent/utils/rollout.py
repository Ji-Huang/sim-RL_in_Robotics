...
def generate_trajectories(
    ...
    *,
    deterministic_policy: bool = False,
    success_episode_only: bool = True
    ...
    for trajectory in trajectories:
        terminal_obs = trajectory.infos[-1].get('trajectory', [])
        if len(terminal_obs) > 0:
            trajectory.obs[-1] = terminal_obs

        if success_episode_only:
            if trajectory.infos[-1]['episode']['success']:
                corrected_trajectories.append(trajectory)
        else:
            corrected_trajectories.append(trajectory)
...
def rollout_stats(
    ...
    out_stats: Dict[str, float] = {"n_traj": len(trajectories),
                                   "n_success": len([traj for traj in trajectories if traj.infos[-1]["episode"]["success"]])}
    traj_descriptors = {
        "return": np.asarray([max(t.rews) for t in trajectories]),
        "len": np.asarray([len(t.rews) for t in trajectories]),
        "iteration_time": np.array([np.mean(list(map(lambda info: info["iteration_time"], t.infos))) for t in trajectories])
    }

    if "pred_error" in trajectories[0].infos[0].keys():
        traj_descriptors["pred_error"] = np.array([np.mean(list(map(lambda info: info["pred_error"], t.infos))) for t in trajectories])
...
def rollout(
    ...
    **kwargs: Any,
    ...
    trajs = generate_trajectories(
        ...
        sample_until,
        rng=rng,
        success_episode_only=success_episode_only,
        **kwargs,
    )
    if unwrap:
...
def shuffle_transitions(transitions: types.Transitions, rng: np.random.Generator):
    transitions_ls = list(transitions)
    rng.shuffle(transitions_ls)

    keys = transitions_ls[0].keys()
    values = map(lambda key: np.array([t[key] for t in transitions_ls]), keys)

    cat_parts = dict(zip(keys, values))
